...
namespace OSnack.API.Controllers
{
   public partial class OrderController
   {
      ...
      public async Task<IActionResult> Post(string paypalId, [FromBody] Order orderData)
      {
         async Task<Order> TryToSave(Order orderData, int tryCount)
         {
            try
            {
               ...
               await _unitOfWork.Orders.AddOrder(orderData).ConfigureAwait(false);
         ...
         try
         {
            ...
            {
               _unitOfWork.Orders.Modify(orderData);
               _unitOfWork.Coupons.Modify(orderData.Coupon);
               await _unitOfWork.CompleteAsync().ConfigureAwait(false);
      ...
      {
         Address currentAddress = await _unitOfWork.Addresses
            .GetAddressByUserId(orderData.AddressId, AppFunc.GetUserId(User)).ConfigureAwait(false);

         if (currentAddress != null)
         {
            orderData.UpdateAddress(currentAddress);
         }
         orderData.DeliveryOption = await _unitOfWork.DeliveryOptions
            .FindAsync(orderData.DeliveryOption.Id)
         ...
         List<Product> productList = await _unitOfWork.Products.GetAllProductIncludeCategory(
            p => orderData.OrderItems.Select(t => t.ProductId).ToList().Contains(p.Id))
           .ConfigureAwait(false);
         ...
         {
            Coupon currentCoupon = await _unitOfWork.Coupons
