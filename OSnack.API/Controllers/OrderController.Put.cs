...
namespace OSnack.API.Controllers
{
   public partial class OrderController
   {
      ...
      public async Task<IActionResult> Put([FromBody] Order modifiedOrder)
      {
         try
         {
            Order originalOrder = await _unitOfWork.Orders.GetOrder(modifiedOrder.Id)
            ...
            switch (originalOrder.Status)
            {
               ...
               case OrderStatusType.FullyRefunded:
                  ...
                  {
                     _unitOfWork.Orders.Modify(originalOrder);
      ...
      {
         List<Product> productList = await _unitOfWork.Products
           .WhereAsync(p => order.OrderItems.Select(t => t.ProductId).Contains(p.Id) == true)
           .ConfigureAwait(false);
         ...
         _unitOfWork.Products.ModifyRange(productList);
      }
   }
}
