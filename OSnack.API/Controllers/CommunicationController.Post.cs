   ...
   public partial class CommunicationController
   {
      ...
      public async Task<IActionResult> PostDispute([FromBody] Communication newDispute)
      {
         try
         {
            ...
            if (!string.IsNullOrEmpty(newDispute.Order_Id) &&
               (await _unitOfWork.Orders.FindAsync(newDispute.Order_Id) == null))
            {
               CoreFunc.Error(ref ErrorsList, "Cannot find your order.");
               return UnprocessableEntity(ErrorsList);
            }

            User user = await _unitOfWork.Users.SingleOrDefaultAsync(u => u.Id == AppFunc.GetUserId(User));
            newDispute.Email = user.Email;
            newDispute.FullName = $"{user.FirstName} {user.Surname}";
            newDispute.Status = true;
            newDispute.Order = await _unitOfWork.Orders.GetOrderWithPaymentAndUser(newDispute.Order_Id)
            ...
            await _unitOfWork.Communications.AddCommunication(newDispute).ConfigureAwait(false);
      ...
      public async Task<IActionResult> PostQuestion([FromBody] Communication newContact)
      {
         ...
         try
         {
            ...
            {
               var user = await _unitOfWork.Users.FindAsync(AppFunc.GetUserId(User));
            ...
            await _unitOfWork.Communications.AddAsync(newContact).ConfigureAwait(false);
