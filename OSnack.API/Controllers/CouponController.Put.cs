   ...
   public partial class CouponController
   {
      ...
      public async Task<IActionResult> Put([FromBody] Coupon modifiedCoupon)
      {
         try
         {
            ...
            if (!await _unitOfWork.Coupons.AnyAsync(d =>
            ...
            if (await _unitOfWork.Coupons.AnyAsync(d => d.Code == modifiedCoupon.Code &&
            ...
            Coupon originalCoupon = await _unitOfWork.Coupons.FindAsync(modifiedCoupon.Code).ConfigureAwait(false);
            originalCoupon.MaxUseQuantity = modifiedCoupon.MaxUseQuantity;
            originalCoupon.ExpiryDate = modifiedCoupon.ExpiryDate;
            originalCoupon.DiscountAmount = modifiedCoupon.DiscountAmount;
            originalCoupon.MinimumOrderPrice = modifiedCoupon.MinimumOrderPrice;

            _unitOfWork.Coupons.Modify(originalCoupon);
